...
template <typename Dtype>
void BoxSegInstancePrefetchingDataLayer<Dtype>::Forward_gpu(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  Batch<Dtype>* batch =
    BasePrefetchingDataLayer<Dtype>::prefetch_full_.pop("Data layer prefetch queue empty");
  // Reshape to loaded data.
  ...
  caffe_copy(batch->instance_seg_.count(), batch->instance_seg_.gpu_data(),
...
INSTANTIATE_LAYER_GPU_FORWARD(BoxSegInstancePrefetchingDataLayer);
