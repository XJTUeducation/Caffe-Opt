...
class Batch {
 public:
    Blob<Dtype> data_, label_, dim_, seg_, instance_seg_;
};
...
template <typename Dtype>
class BoxSegInstancePrefetchingDataLayer : public BasePrefetchingDataLayer<Dtype> {
 public:
  explicit BoxSegInstancePrefetchingDataLayer(const LayerParameter& param)
      : BasePrefetchingDataLayer<Dtype>(param) {}
  virtual ~BoxSegInstancePrefetchingDataLayer() {}
  // LayerSetUp: implements common data layer setup functionality, and calls
  // DataLayerSetUp to do special data layer setup for individual layer types.
  // This method may not be overridden.
  void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
