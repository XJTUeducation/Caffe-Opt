...
__global__ void AvePoolBackward(const int nthreads, const Dtype* const top_diff,
    ...
    Dtype* const bottom_diff, const int accumulate) {
  CUDA_KERNEL_LOOP(index, nthreads) {
    // find out the local index
    // find out the local offset
    ...
    bottom_diff[index] = gradient + bottom_diff[index] * accumulate;
...
void PoolingLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  const int is_bottom_shared = (int)(bottom[0]->is_shared());
  ...
  case PoolingParameter_PoolMethod_AVE:
    ...
    AvePoolBackward<Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
        ...
        height_, width_, pooled_height_, pooled_width_, kernel_h_,
        kernel_w_, stride_h_, stride_w_, pad_h_, pad_w_, bottom_diff, is_bottom_shared);
    break;
  case PoolingParameter_PoolMethod_STOCHASTIC:
    // NOLINT_NEXT_LINE(whitespace/operators)
