...
#endif
            //      std::cout << "PUSHING IN PREFETCH FULL\n";
...
template <typename Dtype>
void BoxSegInstancePrefetchingDataLayer<Dtype>::LayerSetUp(
        const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
    BaseDataLayer<Dtype>::LayerSetUp(bottom, top);
...
//        this->prefetch_[i].seg_.mutable_cpu_data();
//        this->prefetch_[i].instance_seg_.mutable_cpu_data();
    }
#ifndef CPU_ONLY
    if (Caffe::mode() == Caffe::GPU) {
        for (int i = 0; i < BasePrefetchingDataLayer<Dtype>::PREFETCH_COUNT; ++i) {
...
//            this->prefetch_[i].seg_.mutable_gpu_data();
//            this->prefetch_[i].instance_seg_.mutable_gpu_data();
        }
    }
#endif
    DLOG(INFO) << "Initializing prefetch";
...
template <typename Dtype>
void BoxSegInstancePrefetchingDataLayer<Dtype>::Forward_cpu(
        const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
    Batch<Dtype>* batch =
            this->prefetch_full_.pop("Data layer prefetch queue empty");
    // Reshape to loaded data.
    ...
    caffe_copy(batch->seg_.count(), batch->seg_.cpu_data(),
               top[2]->mutable_cpu_data());
    top[3]->ReshapeLike(batch->instance_seg_);
    caffe_copy(batch->instance_seg_.count(), batch->instance_seg_.cpu_data(),
...
STUB_GPU_FORWARD(BoxSegInstancePrefetchingDataLayer, Forward);
#endif

INSTANTIATE_CLASS(BaseDataLayer);
INSTANTIATE_CLASS(BasePrefetchingDataLayer);
INSTANTIATE_CLASS(ImageDimPrefetchingDataLayer);
INSTANTIATE_CLASS(BoxSegInstancePrefetchingDataLayer);
