...
#include "caffe/layers/image_seg_online_data_layer.hpp"
...
ImageSegOnlineDataLayer<Dtype>::~ImageSegOnlineDataLayer<Dtype>() {
    this->StopInternalThread();
}

template <typename Dtype>
void ImageSegOnlineDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
...
//    const int label_type = this->layer_param_.image_data_param().label_type();
    ...
    CHECK(transform_param.has_mean_file() == false) <<
                                                       "ImageSegOnlineDataLayer does not support mean file";
    CHECK((new_height == 0 && new_width == 0) ||
          (new_height > 0 && new_width > 0)) << "Current implementation requires "
                                                "new_height and new_width to be set at the same time.";


    std::string str_id_mapping_file = root_folder + "/id_mapping_file.yaml";

    cv::FileStorage id_mapping_file(str_id_mapping_file,cv::FileStorage::READ);
    cv::Mat mat_id_mapping;

    id_mapping_file["ID_MAPPING"] >> mat_id_mapping;

    for(int i=0;i<mat_id_mapping.rows;i++)
    {
    ...
    std::string dummy_image_file_name = root_folder + "/objects/1.png";

    if(!boost::filesystem::exists(dummy_image_file_name))
    {
        while(!boost::filesystem::exists(dummy_image_file_name))
        {
            std::cout << "\rWAITING FOR THE FILES TO EXIST...";
            usleep(10000);
        }
        std::cout << "\n";
    }
    ...
    cv::Mat cv_img = ReadImageToCVMat(dummy_image_file_name,
                                      new_height, new_width, is_color);

    CHECK(cv_img.data) << "Could not load " << dummy_image_file_name;
    ...
    // MAX IMAGES
    lines_ = this->layer_param_.image_data_param().max_images();
    lines_id_ = 1;
...
template <typename Dtype>
void ImageSegOnlineDataLayer<Dtype>::ShuffleImages() {
...
template <typename Dtype>
void ImageSegOnlineDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
    ...
    for (int item_id = 0; item_id < batch_size; ++item_id) {
        ...
        //------READ IMAGES AND MASK AND KEEP TRACK OF IMAGES AND WHETHER TO GENERATE CLUTTER
        {

            std::stringstream img_file_name;
            std::stringstream seg_file_name;

            img_file_name << root_folder << "/objects/" << lines_id_ << ".png";
            seg_file_name << root_folder << "/annotations/mask_" << lines_id_ << ".png";


            if(!(boost::filesystem::exists(img_file_name.str()) && boost::filesystem::exists(seg_file_name.str())))
            {
                while(!(boost::filesystem::exists(img_file_name.str()) && boost::filesystem::exists(seg_file_name.str())))
                {
                    std::cout << "\rWAITING FOR THE FILES TO EXIST...";
                    usleep(10000);
                }
            ...
            int img_row, img_col;
            cv_img_seg.push_back(PSP_ReadImageToCVMat(img_file_name.str(),
            ...
            if (!cv_img_seg[0].data) {
                DLOG(INFO) << "Fail to load img: " << img_file_name.str();
            }
            if (label_type == ImageDataParameter_LabelType_PIXEL) {
                cv_img_seg.push_back(ReadImageToCVMat(seg_file_name.str(), new_height, new_width, false));
                if (!cv_img_seg[1].data) {
                    DLOG(INFO) << "Fail to load seg: " << seg_file_name.str();
                }
            }
        }
        ...
        double max =0;
        cv::minMaxLoc(mask,NULL,&max);

        std::cout << "MAX mask= " << max << "\n";
...
INSTANTIATE_CLASS(ImageSegOnlineDataLayer);
REGISTER_LAYER_CLASS(ImageSegOnlineData);
