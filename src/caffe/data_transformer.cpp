...
template<typename Dtype>
void DataTransformer<Dtype>::TransformSeg(const Datum& datum,
                                          const Datum& datum_seg,
                                          Blob<Dtype>* transformed_blob,
                                          Blob<Dtype>* transformed_blob_seg,
                                          NormalizedBBox* crop_bbox,
                                          bool* do_mirror
                                          ) {
    // If datum is encoded, decoded and transform the cv::image.
    if (datum.encoded()) {
        ...
        cv_seg = DecodeDatumToCVMat(datum_seg,true);


        // Transform the cv::image into blob.
        return TransformSeg(cv_img,cv_seg, transformed_blob,transformed_blob_seg, crop_bbox, do_mirror);
#else
        LOG(FATAL) << "Encoded datum requires OpenCV; compile with USE_OPENCV.";
#endif  // USE_OPENCV
    } else {
...
template<typename Dtype>
void DataTransformer<Dtype>::TransformSeg(
        const AnnotatedDatum& anno_datum, Blob<Dtype>* transformed_blob,
        Blob<Dtype>* transformed_blob_seg,
        RepeatedPtrField<AnnotationGroup>* transformed_anno_group_all,
        bool* do_mirror) {
    // Transform datum.
    const Datum& datum = anno_datum.datum();
    const Datum& datum_seg = anno_datum.datum_seg();

    NormalizedBBox crop_bbox;
    TransformSeg(datum,datum_seg, transformed_blob,transformed_blob_seg, &crop_bbox, do_mirror);
...
void DataTransformer<Dtype>::TransformSeg(
        ...
        vector<AnnotationGroup>* transformed_anno_vec, bool* do_mirror) {
    RepeatedPtrField<AnnotationGroup> transformed_anno_group_all;
    TransformSeg(anno_datum, transformed_blob,transformed_blob_seg, &transformed_anno_group_all,
                 do_mirror);
    for (int g = 0; g < transformed_anno_group_all.size(); ++g) {
        transformed_anno_vec->push_back(transformed_anno_group_all.Get(g));
    }
...
void DataTransformer<Dtype>::TransformSeg(
        ...
        vector<AnnotationGroup>* transformed_anno_vec) {
    bool do_mirror;
    TransformSeg(anno_datum, transformed_blob,transformed_blob_seg, transformed_anno_vec, &do_mirror);
}
        ...
        for (int g = 0; g < anno_datum.annotation_group_size(); ++g) {
            ...
            for (int a = 0; a < anno_group.annotation_size(); ++a) {
                ...
                if (ProjectBBox(crop_bbox, resize_bbox, &proj_bbox)) {
                    ...
                    proj_bbox.set_label(bbox.label());
...
template<typename Dtype>
void DataTransformer<Dtype>::CropImageSeg(const Datum& datum,
                                          const Datum& datum_seg,
                                          const NormalizedBBox& bbox,
                                          Datum* crop_datum,
                                          Datum* crop_datum_seg) {
    // If datum is encoded, decode and crop the cv::image.
    if (datum.encoded()) {
#ifdef USE_OPENCV
        CHECK(!(param_.force_color() && param_.force_gray()))
        ...
        cv::Mat crop_img;
        cv::Mat crop_seg;
        CropImageSeg(cv_img,cv_seg, bbox, &crop_img,&crop_seg);

        // Save the image into datum.
        EncodeCVMatToDatum(crop_img, "jpg", crop_datum);
        EncodeCVMatToDatum(crop_seg, "png", crop_datum_seg);
        crop_datum->set_label(datum.label());
        crop_datum_seg->set_label(datum.label());
        return;
#else
        LOG(FATAL) << "Encoded datum requires OpenCV; compile with USE_OPENCV.";
#endif  // USE_OPENCV
...
template<typename Dtype>
void DataTransformer<Dtype>::CropImageSeg(const AnnotatedDatum& anno_datum,
                                          const NormalizedBBox& bbox,
                                          AnnotatedDatum* cropped_anno_datum) {
    // Crop the datum.
    CropImageSeg(anno_datum.datum(),
                 anno_datum.datum_seg(),
                 bbox,
                 cropped_anno_datum->mutable_datum(),
                 cropped_anno_datum->mutable_datum_seg());
...
template<typename Dtype>
void DataTransformer<Dtype>::ExpandImageSeg(const Datum& datum,
                                            const Datum& datum_seg,
                                            const float expand_ratio,
                                            NormalizedBBox* expand_bbox,
                                            Datum* expand_datum,
                                            Datum* expand_datum_seg) {
    // If datum is encoded, decode and crop the cv::image.
    if (datum.encoded()) {
#ifdef USE_OPENCV
        CHECK(!(param_.force_color() && param_.force_gray()))
        ...
        // Save the image into datum.
        EncodeCVMatToDatum(expand_img, "jpg", expand_datum);
        EncodeCVMatToDatum(expand_seg, "png", expand_datum_seg);

        expand_datum->set_label(datum.label());
        expand_datum_seg->set_label(datum_seg.label());
...
template<typename Dtype>
void DataTransformer<Dtype>::ExpandImageSeg(const AnnotatedDatum& anno_datum,
                                            AnnotatedDatum* expanded_anno_datum) {
    if (!param_.has_expand_param()) {
        expanded_anno_datum->CopyFrom(anno_datum);
        return;
    ...
    // Expand the datum.
    NormalizedBBox expand_bbox;
    ExpandImageSeg(anno_datum.datum(),anno_datum.datum_seg(), expand_ratio, &expand_bbox,
                   expanded_anno_datum->mutable_datum(),
                   expanded_anno_datum->mutable_datum_seg());
    expanded_anno_datum->set_type(anno_datum.type());
...
template<typename Dtype>
void DataTransformer<Dtype>::TransformSeg(const cv::Mat& cv_img,
                                          const cv::Mat& cv_seg,
                                          Blob<Dtype>* transformed_blob,
                                          Blob<Dtype>* transformed_blob_seg,
                                          NormalizedBBox* crop_bbox,
    ...
    *do_mirror = param_.mirror() && Rand(2);
    ...
    cv::Mat cv_resized_image, cv_noised_image, cv_cropped_image;
    cv::Mat cv_resized_seg, cv_cropped_seg;

    if (param_.has_resize_param()) {
        cv_resized_image = ApplyResize(cv_img, param_.resize_param());
        int resize_h = param_.resize_param().height();
        int resize_w = param_.resize_param().width();

        cv::resize(cv_seg,cv_resized_seg,cv::Size(resize_w,resize_h),0,0,CV_INTER_NN);
    } else {
        cv_resized_image = cv_img;
        cv_resized_seg = cv_seg;
    }
    if (param_.has_noise_param()) {
        cv_noised_image = ApplyNoise(cv_resized_image, param_.noise_param());
    } else {
    ...
    if ((crop_h > 0) && (crop_w > 0)) {
        ...
        cv::Rect roi(w_off, h_off, crop_w, crop_h);
        cv_cropped_image = cv_noised_image(roi);
        cv_cropped_seg = cv_resized_seg(roi);
    } else {
        cv_cropped_image = cv_noised_image;
        cv_cropped_seg = cv_resized_seg;
    }
    ...
    Dtype* transformed_data = transformed_blob->mutable_cpu_data();
    Dtype* transformed_data_seg = transformed_blob_seg->mutable_cpu_data();


    int top_index;
    for (int h = 0; h < height; ++h) {
        const uchar* ptr = cv_cropped_image.ptr<uchar>(h);
        const uchar* ptr_seg = cv_cropped_seg.ptr<uchar>(h);
        int img_index = 0;
        int seg_index = 0;
        int h_idx = h;
        for (int w = 0; w < width; ++w) {
            ...
            for (int c = 0; c < img_channels; ++c) {
                ...
                transformed_data_seg[top_index] = pixel_seg;
            }
        }
    }
}
...
template <typename Dtype>
void DataTransformer<Dtype>::CropImageSeg(const cv::Mat& img,
                                          const cv::Mat& seg,
                                          const NormalizedBBox& bbox,
                                          cv::Mat* crop_img,
                                          cv::Mat* crop_seg) {
...
template <typename Dtype>
void DataTransformer<Dtype>::ExpandImageSeg(const cv::Mat& img,
                                            const cv::Mat& seg,
                                            const float expand_ratio,
                                            NormalizedBBox* expand_bbox,
                                            cv::Mat* expand_img,
    ...
    expand_img->create(height, width, img.type());
    expand_img->setTo(cv::Scalar(0));
    expand_seg->create(height, width, img.type());
    expand_seg->setTo(cv::Scalar(0));
    const bool has_mean_file = param_.has_mean_file();
    const bool has_mean_values = mean_values_.size() > 0;
...
template<typename Dtype>
void DataTransformer<Dtype>::TransformImg(
        const cv::Mat& cv_img,
    ...
    uint64 random_seed = (uint64)std::time(0);

    static cv::RNG rng(random_seed);


    // Do random brightness distortion.
    int apply_brightness = rng(2);
    if(param_.has_rand_brightness() && param_.rand_brightness() && apply_brightness)
    {
        RandomBrightness((const cv::Mat&)cv_img, (cv::Mat*)(&cv_img), param_.brightness_prob(),
                         param_.brightness_delta());
    }


    // Do random contrast distortion.
    int apply_contrast = rng(2);
    if(param_.has_rand_contrast() && param_.rand_contrast() && apply_contrast)
    {
        RandomContrast((const cv::Mat&)cv_img, (cv::Mat*)(&cv_img), param_.contrast_prob(),
                       param_.contrast_lower(), param_.contrast_upper());
    }
    // Do random saturation distortion.
    int apply_saturation = rng(2);
    if(param_.has_rand_saturation() && param_.rand_saturation() && apply_saturation)
    {
        RandomSaturation((const cv::Mat&)cv_img, (cv::Mat*)(&cv_img), param_.saturation_prob(),
                         param_.saturation_lower(), param_.saturation_upper());
    }
    // Do random hue distortion.
    int apply_hue = rng(2);
    if(param_.has_rand_hue() && param_.rand_hue() && apply_hue)
    {
        RandomHue((const cv::Mat&)cv_img, (cv::Mat*)(&cv_img), param_.hue_prob(), param_.hue_delta());
    }

    int apply_smooth = rng(2);
    if ( param_.has_rand_gaussian_blur() && apply_smooth ) {

        // gaussian blur
        int smooth_param = param_.gaussian_smooth_param();

        smooth_param += 2*rng(2);
        cv::GaussianBlur(cv_img, cv_img, cv::Size(smooth_param,smooth_param),0);
    }

    // rotation
    float rotation_degree;
    int apply_rotation = rng(2);
    if (param_.has_rand_rotate() && apply_rotation)
    {
        float rand_rotate_min = param_.img_rot_min();
        float rand_rotate_max = param_.img_rot_max();

        rotation_degree = rng.uniform(rand_rotate_min,rand_rotate_max);
        cv::Mat r = getRotationMatrix2D(cv::Point2i(cv_img.cols/2.0f,cv_img.rows/2.0f), rotation_degree, 1.0);
        cv::warpAffine(cv_img, cv_img, r, cv::Size(cv_img.cols,cv_img.rows),cv::INTER_LINEAR);
    }
    ...
    cv::Mat cv_img_loc = cv_img;

    cv_img_loc.convertTo(cv_img_loc, CV_64F);
    ...
    for (int h = 0; h < data_height; ++h) {
        data_ptr = cv_img_loc.ptr<double>(h);
...
template<typename Dtype>
void DataTransformer<Dtype>::TransformImgAndSegAll(
        const std::vector<cv::Mat>& cv_img_seg,
        Blob<Dtype>* transformed_data_blob,
        Blob<Dtype>* transformed_label_blob,
        const int ignore_label) {
    ...
    if(param_.has_rand_brightness() && param_.rand_brightness() && apply_brightness)
    {
        RandomBrightness((const cv::Mat&)cv_img_seg[0], (cv::Mat*)(&cv_img_seg[0]), param_.brightness_prob(),
                param_.brightness_delta());
    }
    ...
    if(param_.has_rand_contrast() && param_.rand_contrast() && apply_contrast)
    {
        RandomContrast((const cv::Mat&)cv_img_seg[0], (cv::Mat*)(&cv_img_seg[0]), param_.contrast_prob(),
                param_.contrast_lower(), param_.contrast_upper());
    }
    // Do random saturation distortion.
    int apply_saturation = rng(2);
    if(param_.has_rand_saturation() && param_.rand_saturation() && apply_saturation)
    {
        RandomSaturation((const cv::Mat&)cv_img_seg[0], (cv::Mat*)(&cv_img_seg[0]), param_.saturation_prob(),
                param_.saturation_lower(), param_.saturation_upper());
    }
    // Do random hue distortion.
    int apply_hue = rng(2);
    if(param_.has_rand_hue() && param_.rand_hue() && apply_hue)
    {
        RandomHue((const cv::Mat&)cv_img_seg[0], (cv::Mat*)(&cv_img_seg[0]), param_.hue_prob(), param_.hue_delta());
    }


    // gaussian blur
    int apply_smooth = rng(2);
    if (param_.has_rand_gaussian_blur() && param_.rand_gaussian_blur() && apply_smooth ) {

        int smooth_param = param_.gaussian_smooth_param();

        smooth_param += 2*rng(2);
        cv::GaussianBlur(cv_img_seg[0], cv_img_seg[0], cv::Size(smooth_param,smooth_param),0);
    }
    ...
    float rotation_degree;
    int apply_rotation = rng(2);
    if (param_.has_rand_rotate() && param_.rand_rotate() && apply_rotation)
    {
        float rand_rotate_min = param_.img_rot_min();
        float rand_rotate_max = param_.img_rot_max();

        rotation_degree = rng.uniform(rand_rotate_min,rand_rotate_max);
        cv::Mat r = getRotationMatrix2D(cv::Point2i(cv_img_seg[0].cols/2.0f,cv_img_seg[0].rows/2.0f), rotation_degree, 1.0);
        cv::warpAffine(cv_img_seg[0], cv_img_seg[0], r, cv::Size(cv_img_seg[0].cols, cv_img_seg[0].rows),cv::INTER_LINEAR);
        cv::warpAffine(cv_img_seg[1], cv_img_seg[1], r, cv::Size(cv_img_seg[1].cols, cv_img_seg[1].rows),
                cv::INTER_NEAREST, cv::BORDER_CONSTANT, cv::Scalar(ignore_label));
    }
...
template<typename Dtype>
void DataTransformer<Dtype>::TransformImgAndSegOnline(
        const std::vector<cv::Mat>& cv_img_seg,
        Blob<Dtype>* transformed_data_blob,
        Blob<Dtype>* transformed_label_blob,
        const int ignore_label) {
    ...
    // scale
    //    float rs_scale;
    //    int apply_scale = rng(2);

    //    if(has_rand_scale && apply_scale)
    //    {
    //        float rand_scale_min = rand_scale[0];
    //        float rand_scale_max = rand_scale[1];

    //        rs_scale = rng.uniform(rand_scale_min,rand_scale_max);
    //        std::cout << "SCALE = " << rs_scale << "\n";
    //        if(rs_scale != 1.0&& cv_img_seg[0].data)
    //        {
    //            cv::Mat rs_1;
    //            cv::Mat rs_2;
    //            cv_img_seg[0].copyTo(rs_1);
    //            cv_img_seg[1].copyTo(rs_2);
    ...
    //                 cv::resize(rs_1,rs_1,cv::Size(img_width,img_height),rs_scale,rs_scale,cv::INTER_LINEAR);
    //                cv::resize(rs_2,rs_2,cv::Size(img_width,img_height),rs_scale,rs_scale,cv::INTER_NEAREST);

    //        }
    //    }

    //    cv::imshow("image_new_color",cv_img_seg[0]);
    //    cv::imshow("image_new_seg",cv_img_seg[1]);
    ...
    //    cv::imshow("live",cv_cropped_img);
    //    cv::imshow("seg",5*cv_cropped_seg);
    //    cv::waitKey(0);
    ...
    //    cv::imshow("image_new_color cr",cv_cropped_img);
    //    cv::imshow("image_new_segcv",cv_cropped_seg);
...
void DataTransformer<Dtype>::InitRand() {
    const bool needs_rand = param_.mirror() ||
            (phase_ == TRAIN && (param_.has_crop_size() || (param_.has_crop_height() && param_.has_crop_width())));
    if (needs_rand) {
