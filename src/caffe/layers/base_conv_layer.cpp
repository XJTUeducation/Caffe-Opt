...
unsigned long int BaseConvolutionLayer<Dtype>::col_buffer_count_ = 0;

template <typename Dtype>
std::map<int, Dtype*>* BaseConvolutionLayer<Dtype>::pointer_map_ = NULL;
...
void BaseConvolutionLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
    ...
    // GLOBAL COL BUFFER
    {
        ...
        if(total_count > col_buffer_count_)
        {
            col_buffer_count_ = total_count;
            ...
            cudaMalloc(&gpu_col_buffer,col_buffer_count_ * sizeof(Dtype));
...
void BaseConvolutionLayer<Dtype>::forward_gpu_gemm(const Dtype* input,
    ...
    unsigned long int total_buffer_required = (unsigned long int )kernel_shape_.cpu_data()[0] *  kernel_shape_.cpu_data()[1] * conv_in_channels_ * conv_out_spatial_dim_;
    ...
    unsigned long int avail_buffer = col_buffer_count_;


#else

    Dtype* gpu_col_buff = temp_buffer_.mutable_gpu_data();
    unsigned long int avail_buffer = temp_buffer_.data()->size() / sizeof(Dtype);
    ...
    else
    {
        ...
        unsigned long int  per_loc_mem_required = kernel_shape_.cpu_data()[0] * kernel_shape_.cpu_data()[1] * conv_in_channels_;

        int n_loc_can_be_processed = avail_buffer / per_loc_mem_required;

        if(n_loc_can_be_processed > conv_out_spatial_dim_)
            n_loc_can_be_processed = conv_out_spatial_dim_;

        int n_partition = ((conv_out_spatial_dim_ - 1) / n_loc_can_be_processed) + 1;
        ...
        for(int i=0; i < n_partition; i++)
        {
            ...
            for (int g = 0; g < group_; ++g)
            {
                int col_offset = to_process * kernel_dim_;

                caffe_gpu_gemm_ld<Dtype>(CblasNoTrans, CblasNoTrans, conv_out_channels_ / group_,
                                         to_process, kernel_dim_,
                                         (Dtype)1., weights + weight_offset_ * g, col_buff  + col_offset * g,
                                         (Dtype)0., output + offset + output_offset_ * g,-1,-1, conv_out_spatial_dim_);
...
void BaseConvolutionLayer<Dtype>::backward_gpu_gemm(const Dtype* output,
    ...
    unsigned long int total_buffer_required = (unsigned long int )kernel_shape_.cpu_data()[0] * kernel_shape_.cpu_data()[1] *
    ...
    else
    {
        ...
        // each location corresponds to an input channel
        int n_loc_can_be_processed =  avail_buffer / per_loc_mem_required;

        if(n_loc_can_be_processed > (conv_in_channels_ / group_))
            n_loc_can_be_processed = (conv_in_channels_  / group_);

        int n_partition = (((conv_in_channels_ / group_) - 1) / n_loc_can_be_processed) + 1;

        //        std::cout << "N PARTITION = " << n_partition << "\n";

        unsigned long int kernel_extent = (unsigned long int )kernel_shape_.cpu_data()[0] * kernel_shape_.cpu_data()[1];
        ...
        for (int g = 0; g < group_; ++g)
        {
            ...
            for(int i=0; i < n_partition; i++)
            {
                n_loc_remaining = conv_in_channels_  / group_ - n_loc_processed;
                ...
                caffe_gpu_gemm_ld<Dtype>(CblasTrans, CblasNoTrans, kernel_extent * to_process,
                                         conv_out_spatial_dim_, conv_out_channels_ / group_,
                                         (Dtype)1., weights + weight_offset + weight_offset_ * g, output + output_offset_ * g,
                                         (Dtype)0., col_buff , kernel_dim_, -1 , -1 );


                const int input_spatial_dim = conv_input_shape_.cpu_data()[1] * conv_input_shape_.cpu_data()[2];
                const int input_offset = (unsigned long int )n_loc_processed * input_spatial_dim;
                Dtype* input_ = input + ( conv_in_channels_ / group_ ) * input_spatial_dim * g +  input_offset;
                ...
                col2im_gpu(col_buff, to_process,
                        ...
                        pad_.cpu_data()[0], pad_.cpu_data()[1],
                        stride_.cpu_data()[0], stride_.cpu_data()[1],
                        dilation_.cpu_data()[0], dilation_.cpu_data()[1], input_, accumulate);
...
void BaseConvolutionLayer<Dtype>::weight_gpu_gemm(const Dtype* input,
    ...
    else
    {
        ...
        //        std::cout << "N PARTITION gemm = " << n_partition << "\n";
        ...
        for(int i=0; i < n_partition; i++)
        {
            ...
            for (int g = 0; g < group_; ++g)
            {
                ...
                caffe_gpu_gemm_ld<Dtype>(CblasNoTrans, CblasTrans, conv_out_channels_ / group_,
                                         kernel_dim_, to_process,
                                         (Dtype)1., output + offset + output_offset_ * g , col_buff + col_offset * g,
                                         (Dtype)1., weights + weight_offset_ * g , conv_out_spatial_dim_, -1,-1 );
            }
...
//        int n_partition = ceil((double)(conv_in_channels_ / group_)/n_loc_can_be_processed);
...
//                                         conv_out_spatial_dim_, conv_out_channels_ / group_,
//                                         (Dtype)1., weights + weight_offset + weight_offset_ * g, output + output_offset_ * g,
//                                         (Dtype)0., col_buff + col_offset_ * g, kernel_dim_, -1 , -1 );
//            }
...
//                caffe_gpu_gemm_ld<Dtype>(CblasNoTrans, CblasTrans, conv_out_channels_ / group_,
//                                         kernel_dim_, to_process,
//                                         (Dtype)1., output + offset + output_offset_ * g , col_buff + col_offset_ * g,
//                                         (Dtype)1., weights + weight_offset_ * g , conv_out_spatial_dim_, -1,-1 );
//            }
