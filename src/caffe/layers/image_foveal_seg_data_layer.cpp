...
#include "caffe/layers/image_foveal_seg_data_layer.hpp"
...
ImageFovealSegDataLayer<Dtype>::~ImageFovealSegDataLayer<Dtype>() {
    this->StopInternalThread();
}

template <typename Dtype>
void ImageFovealSegDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
    ...
    const int foveal_roi_h = this->layer_param_.image_data_param().foveal_roi_h();
    const int foveal_roi_w = this->layer_param_.image_data_param().foveal_roi_w();
    ...
    if (crop_width > 0 && crop_height > 0) {
        ...
        //label
        top[1]->Reshape(batch_size, 1, foveal_roi_h, foveal_roi_w);
        this->transformed_label_.Reshape(1, 1, crop_height, crop_width);
        for (int i = 0; i < this->PREFETCH_COUNT; ++i) {
            this->prefetch_[i].label_.Reshape(batch_size, 1, foveal_roi_h, foveal_roi_w);
        }
    } else {
...
template <typename Dtype>
void ImageFovealSegDataLayer<Dtype>::ShuffleImages() {
...
template <typename Dtype>
void ImageFovealSegDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
    ...
    const int cropped_height = batch->data_.shape()[2];
    const int cropped_width = batch->data_.shape()[3];


    const int foveal_roi_y = (cropped_height - foveal_roi_h ) / 2;
    const int foveal_roi_x = (cropped_width - foveal_roi_w ) / 2;
    ...
    for (int item_id = 0; item_id < batch_size; ++item_id) {
        ...
        this->data_transformer_->TransformImgAndSegAll(cv_img_seg,
      ...
      offset = batch->label_.offset(item_id);
        ...
        Dtype* foveal_roi_label = top_label + offset;
        const Dtype* cropped_label = this->transformed_label_.cpu_data();


        for(int i=0;i<foveal_roi_h;i++)
            for(int j=0;j<foveal_roi_w;j++)
                 foveal_roi_label[i * foveal_roi_w + j] = cropped_label[ (foveal_roi_y + i) * cropped_width + foveal_roi_x + j];



//        cv::Mat img = cv_img_seg[0];
//        cv::Mat mask = cv_img_seg[1](cv::Rect((cv_img_seg[0].cols - cropped_width) / 2, (cv_img_seg[0].rows - cropped_height) / 2, cropped_width, cropped_height ));
//        cv::Mat foveal_mask = cv::Mat::zeros(foveal_roi_h, foveal_roi_w, CV_8UC1);

//        for(int i=0;i<foveal_mask.rows * foveal_mask.cols;i++)
//            foveal_mask.data[i] = foveal_roi_label[i];

//        cv::Mat foveal_mask_org = mask(cv::Rect(foveal_roi_x, foveal_roi_y, foveal_roi_w, foveal_roi_h));


//        cv::imshow("live",cv_img_seg[0]);
//        cv::imshow("seg",5*cv_img_seg[1]);
//        cv::imshow("o",5*foveal_mask);
//        cv::imshow("f",5*foveal_mask_org);


//        std::cout << foveal_mask << "\n\n";
//        std::cout << foveal_mask_org << "\n\n";
...
INSTANTIATE_CLASS(ImageFovealSegDataLayer);
REGISTER_LAYER_CLASS(ImageFovealSegData);
