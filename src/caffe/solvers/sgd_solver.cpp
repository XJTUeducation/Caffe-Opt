...
//      if( (this->iter_ % shot_iter) >= shot_iter_min && (this->iter_ % shot_iter) <= shot_iter_max )
//      {
//          rate = .05;//this->param_.base_lr();
//      }
    ...
    rate = this->param_.base_lr() *
      ...
      static Dtype first_loss = 0;

      if(is_first)
      {
          first_loss = this->smoothed_loss_;
          is_first = false;
      }

      rate = this->param_.base_lr() * (this->smoothed_loss_ / first_loss);
   }
  else {
